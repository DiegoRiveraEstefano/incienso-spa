...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        return get_object_or_404(Cart, user=self.request.user.id)
    ...
    @action(methods=['POST'], detail=False, permission_classes=[AllowAny, ], url_name='add-product')
    def add_product(self, request: Request, *args, **kwargs):
        data = request.data
    ...
    @action(methods=['post'], detail=True, permission_classes=[IsAuthenticated], url_name='remove-product')
